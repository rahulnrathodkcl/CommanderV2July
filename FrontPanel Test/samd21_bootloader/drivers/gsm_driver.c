...
enum gsm_error gsm_call_to_dial_a_number(const uint8_t *to)
{
	...
	err = gsm_send_at_command((const char*)cmdx, (const char*)RESPONS_OK,1000,0, NULL);
	free(cmdx);
	return err;
...
enum gsm_error gsm_play_record_file(const char *filename,bool playInfinitely)
{
	...
	memset(cmdx, '\0', MAX_BUFFER);
	snprintf((char*)cmdx, MAX_BUFFER, "AT+CREC=4,\"C:\\User\\%c.amr\",0,90,%c\r",filename,repeat);
